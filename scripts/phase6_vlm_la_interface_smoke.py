...
This smoke test reads Phase 5R real-sensor candidate artifacts, generates
...
for invalid outputs. It does not run real VLM inference, move A1, map, generate
new candidates, train, or modify the USD scene.
"""
...
PHASE = "Phase 6 VLM-LA interface smoke"
...
TOP_REPORT = RUNS / "VLM_LA_INTERFACE_SMOKE_REPORT.md"
...
def find_phase5r_run(explicit_run_dir: str | None = None) -> Path:
    if explicit_run_dir:
        run_dir = Path(explicit_run_dir).expanduser().resolve()
        if run_dir.exists():
            return run_dir
        raise FileNotFoundError(f"explicit Phase 5R run_dir not found: {run_dir}")

    report = RUNS / "A1_REAL_SENSOR_CANDIDATE_GAIN_REPORT.md"
    ...
    matches = sorted(
        RUNS.glob("phase5r_a1_real_sensor_candidate_gain_smoke_*"),
        key=lambda p: p.stat().st_mtime,
        reverse=True,
    )
    if matches:
        return matches[0]
    raise FileNotFoundError("No Phase 5R real-sensor candidate run directory found")


def load_phase5r_artifacts(run_dir: Path) -> tuple[dict[str, Any], list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    if not summary_path.exists() or not csv_path.exists() or not steps_path.exists():
        raise FileNotFoundError(f"Phase 5R candidate artifacts incomplete under {run_dir}")
    ...
    if not rows or not steps:
        raise RuntimeError("Phase 5R candidate artifacts are empty")
...
def build_candidate_index(rows: list[dict[str, Any]]) -> tuple[dict[tuple[int, int], dict[str, Any]], dict[int, dict[str, Any]], dict[int, list[dict[str, Any]]]]:
...
def parse_language_command(text: str) -> dict[str, Any]:
    ...
    lower = raw.lower()
    if re.search(r"\bv\s*=", lower) or "omega" in lower:
        return {"parse_success": False, "selected_candidate_id": None, "error": "velocity_output_not_allowed"}
    if "joint" in lower or "hip joint" in lower or "knee joint" in lower:
        return {"parse_success": False, "selected_candidate_id": None, "error": "joint_action_output_not_allowed"}
    if re.search(r"\bx\s*=", lower) or re.search(r"\by\s*=", lower) or "yaw" in lower and "candidate" not in lower and raw.startswith("{"):
    ...
    if raw.startswith("{"):
        ...
        if any(key in payload for key in ("x", "y", "z", "yaw", "v", "omega")):
...
def validate_candidate_id(candidate_id: int | None, step_id: int, candidate_index: dict[tuple[int, int], dict[str, Any]]) -> dict[str, Any]:
...
def make_test_cases(step_id: int, selected_id: int, rows_for_step: list[dict[str, Any]]) -> list[dict[str, Any]]:
    tests = [
        ...
        {"test_case": "legal_explanation_command", "input_command": f"Go to candidate {selected_id} because it faces unknown space.", "expected": "legal"},
        {
...
def summarize(
    run_dir: Path,
    phase5r_run_dir: Path,
    phase5r_summary: dict[str, Any],
    ...
    legal_valid = [r for r in legal if r["candidate_exists"] and r["candidate_valid"] and r["candidate_reachable"] and r["candidate_collision_risk_ok"]]
    ...
    invalid_fallback = any(r["test_case"] == "invalid_candidate_id" and r["fallback_used"] and r["interface_passed"] for r in records)
    unreachable_fallback = any(r["test_case"] == "unreachable_candidate_id" and r["fallback_used"] and r["interface_passed"] for r in records)
    pass_ok = bool(
        phase5r_summary.get("safe_to_continue_phase6")
        and legal
        and illegal
        ...
        and all(r["final_interface_command"] and re.match(r"^Go to candidate \d+\.$", r["final_interface_command"]) for r in records)
    )
    return {
        "phase": PHASE,
        ...
        "phase5r_run_dir": str(phase5r_run_dir),
        "phase5r_candidate_data_used": True,
        "phase5r_safe_to_continue_phase6": bool(phase5r_summary.get("safe_to_continue_phase6")),
        ...
        "invalid_candidate_fallback_tested": bool(invalid_fallback),
        "unreachable_candidate_fallback_tested": bool(unreachable_fallback),
        ...
        "safe_to_continue_phase7": pass_ok,
        "run_dir": str(run_dir),
        ...
        "caveats": [
            ...
            "Candidate data is read from Phase 5R real-sensor artifacts; no new candidates are generated.",
            "Fallback uses the Phase 5R classical selected candidate for the same step.",
        ],
    }
...
def write_report(path: Path, summary: dict[str, Any]) -> None:
    lines = [
        ...
        f"phase5r_candidate_data_used: {bool_text(summary.get('phase5r_candidate_data_used'))}",
        ...
        f"fallback_behavior: {'pass' if summary.get('fallback_test_passed') else 'fail'}",
        ...
        f"safe_to_continue_phase7: {bool_text(summary.get('safe_to_continue_phase7'))}",
        ...
        "## Evidence",
        "",
        f"- phase5r_run_dir: {summary.get('phase5r_run_dir')}",
...
def main() -> int:
    ...
    parser.add_argument("--phase5r_run_dir", default=None)
    parser.add_argument("--top_report", default=str(TOP_REPORT))
    args = parser.parse_args()

    run_dir = Path(args.run_dir).expanduser().resolve()
    phase5r_run_dir = find_phase5r_run(args.phase5r_run_dir)
    ...
    summary: dict[str, Any] = {
        ...
        "workspace": str(WORKSPACE),
        "project_name": PROJECT_NAME,
        "safe_to_continue_phase7": False,
    }
    exit_code = 1
    started = time.time()
    try:
        phase5r_summary, rows, steps = load_phase5r_artifacts(phase5r_run_dir)
        ...
        summary_path = run_dir / "summary/parse_summary.json"
        test_csv = run_dir / "test_cases/interface_test_cases.csv"
        write_jsonl(jsonl_path, records)
        write_test_case_csv(test_csv, records)
        summary = summarize(run_dir, phase5r_run_dir, phase5r_summary, records, jsonl_path)
        exit_code = 0 if summary["safe_to_continue_phase7"] else 2
        ...
        write_report(run_dir / "reports/VLM_LA_INTERFACE_SMOKE_REPORT.md", summary)
