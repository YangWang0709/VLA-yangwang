...
- Created `scripts/phase4r_a1_real_sensor_mapping_smoke.py`.
- Opened the primary USD scene without saving or overwriting it.
- Used existing `/World/A1` and `/World/A1/base`.
- Reused the Phase 5.6 real Isaac/Omniverse RGB-D route.
- Captured real RGB, depth, camera params, and intrinsics.
- Built depth-backprojected pointclouds and converted them to world-frame points.
- Updated a BEV occupancy map from depth-backprojected pointclouds only.
- Recorded semantic/instance segmentation and RTX LiDAR as optional channels.
- Wrote `runs/A1_REAL_SENSOR_MAPPING_SMOKE_REPORT.md`.
...
- This is mapping smoke only, not candidate generation and not Phase 6.
- Geometry proxy and mounted geometry proxy were not used.
- Next step is rerunning Phase 5 with real sensors.
